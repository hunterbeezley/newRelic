#!/usr/bin/env python3
"""
Filters users from JSON based on createdAt date (eg. older than 1 month)
and outputs unique user IDs in the format expected for the accomanying script massDeleteUsers.py
"""
...
class UserFilter:
    ...
    def parse_date(self, date_value: Any) -> datetime:
        ...
        Expected format: 1669662687473 (milliseconds since epoch)
        """
        if not date_value:
            return None
        ...
        if isinstance(date_value, (int, float)):
            try:
                ...
                timestamp_seconds = date_value / 1000
    ...
    def extract_created_date(self, user_obj: Dict[str, Any]) -> datetime:
        """
        Extract created_at date from user object.
        Expected key: 'created_at' with Unix timestamp in milliseconds
        """
        if 'created_at' in user_obj and user_obj['created_at']:
            return self.parse_date(user_obj['created_at'])
